...
<head>
  ...
  <style>
    .fa-solid {
      font-family: "Dosis", "Font Awesome 6 Free";
    }
  </style>
...
<body>
  ...
  <div class="mt">
    <h2 class="aaa"><i class="fa-solid fa-coins"></i> My Loans</h2>
    <h4 class="aaaa">All of your taken loans shown here</h4>
    <div class="overflow-auto">
      <table id="example" class="table table-borderless table-hover table-striped" style="width:100%">

        <thead class="table-dark ccc">
          <th>Loan Giver Image</th>
          <th>Loan Giver Name</th>
          <th>Loan Giver ID</th>
          <th>Loan Giver Email</th>
          <th>Amount</th>
          <th>Interest</th>
          <th>Amount With Interest</th>
          <th>Deadline</th>
          <th>Amount Due</th>
          <th>Payment from Loan Giver</th>
          
          <th>Pay to Loan Giver</th>
        ...
        <tbody>
          ...
          include '_dbconnect.php';
          $email = $_SESSION['email'];
          $q =  "SELECT * FROM `loan` where loan_taker='$email';";
          ...
          if (mysqli_num_rows($run) > 0) {
            ...
            while ($row = $run->fetch_assoc()) {
              ...
              $g_email = $row["loan_giver"];
              // if($row["deadline"]>date('Y-m-d') && $row["due"]>0 ){
              //   $val = (int)$stk;
              //   $val = $val + 1;
                
              //   $qfff =  "UPDATE `user` SET `rating`='$val' WHERE email='$g_email';";
              //   $runfff = mysqli_query($con, $qfff);
              // }
              
              $q2 =  "SELECT * FROM `user` where email='$g_email';";
              ...
              <tr><td>
                ...
                <?php if($row["payment"]=="Not Paid" && $row["due"]>0){ ?>
                <button style="color: white;" class="btn btn-lg btn-block btn btn-dark w-100" disabled><i class="fa-solid fa-spinner"></i> Payment is Processing</button>
                <?php } ?>
                <?php if($row["payment"]=="Paid" && $row["due"]>0){ ?>
                  <a style="color: white;" href="payment.php?pid=<?php echo $row["loan_id"] ?>" class="btn btn-lg btn-block btn btn-info w-100"> ৳ Pay</a>
                  <?php } ?>
                  <?php if($row["payment"]=="Paid" && $row["due"]<=0){ ?>
                <button style="color: white;" class="btn btn-lg btn-block btn btn-dark w-100" disabled><i class="fa-solid fa-check"></i> Paid</button>
                <?php } ?>
              </td></tr>
                <?php }
          }
